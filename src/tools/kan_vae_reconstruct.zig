...
pub fn main() !void {
    ...
    if (args.len < 4) {
        std.debug.print("Usage: {s} <base.obj> <vae_model.kan> <output.obj> [z0 z1 ... z15]\n", .{args[0]});
        return;
    }
    ...
    const latent_dim = net.layers[0].in_dim - 2;
    var latents = try allocator.alloc(f32, latent_dim);
    defer allocator.free(latents);
    @memset(latents, 0.0);

    // Parse latents if provided
    for (0..latent_dim) |i| {
        if (args.len > 4 + i) {
            latents[i] = try std.fmt.parseFloat(f32, args[4 + i]);
        }
    }
    ...
    std.debug.print("VAE-KAN Reconstruct: in_dim={d}, latent_dim={d}\n", .{net.layers[0].in_dim, latent_dim});

    // 3. Export Displaced Mesh
    ...
    while (f_idx < faces.items.len) {
        ...
        for (0..current_batch_faces) |i| {
            ...
            for (0..3) |j| {
                ...
                activations[0][(i * 3 + j) * net.layers[0].in_dim + 0] = uv.u;
                activations[0][(i * 3 + j) * net.layers[0].in_dim + 1] = uv.v;
                for (0..latent_dim) |k| {
                    activations[0][(i * 3 + j) * net.layers[0].in_dim + 2 + k] = latents[k];
                }
            }
        }
        ...
        for (0..current_batch_faces) |i| {
            ...
            for (0..3) |j| {
                ...
                const displaced_pos = pos.add(.{ .x = net_out[0], .y = net_out[1], .z = net_out[2] });
    ...
    std.debug.print("Saved sanity check to {s}\n", .{out_path});
}
