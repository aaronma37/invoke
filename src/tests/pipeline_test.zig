...
test "Pipeline: Basic UV Displacement Training" {
    ...
    for (0..5000) |_| {
        for (0..batch_size) |i| {
            inputs[i * 2 + 0] = rand.float(f32); // u
            inputs[i * 2 + 1] = rand.float(f32); // v
            targets[i] = 0.7;
        }
        const batch = TrainingBatch{ .inputs = inputs, .targets = targets, .batch_size = batch_size };
        _ = try trainer.trainStep(batch);
    ...
    // Verify a random UV point (should be approx 0.7)
    ...
    for (0..trainer.net.layers.len) |i| test_acts[i] = try allocator.alloc(f32, trainer.net.layers[i].in_dim);
