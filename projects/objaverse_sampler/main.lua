...
function M.init()
    ...
    print("GPU Sampler: Signed Distance Mode (SDF)")
    local raw_data, vertex_count = loader.load(obj_path)
    if not raw_data then error("Failed to load mesh: " .. obj_path) end
    ...
    print(string.format("Normalization: Center(%.2f,%.2f,%.2f) Scale(%.2f)", center[1],center[2],center[3], scale))
    ...
    -- 3. Prepare Random Samples
    local host_samples = ffi.new("PointSample[?]", num_samples)
    for i=0, num_samples-1 do
        -- 50% on surface, 50% in volume
        if i < num_samples / 2 then
    ...
    -- 4. Pipeline
    local l = descriptors.create_layout(d, {{binding=0, type=vk.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, stages=vk.VK_SHADER_STAGE_COMPUTE_BIT},{binding=1, type=vk.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, stages=vk.VK_SHADER_STAGE_COMPUTE_BIT},{binding=2, type=vk.VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, stages=vk.VK_SHADER_STAGE_COMPUTE_BIT}})
    ...
    local ds = descriptors.allocate_sets(d, descriptors.create_pool(d, {{type=vk.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, count=2},{type=vk.VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, count=1}}), {l})[1]
    ...
    -- 5. Dispatch
    print("Sampling " .. num_samples .. " points on GPU...")
